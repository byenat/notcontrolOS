...
import { ContextWindowManager, ContextSegment, OptimizedContext, TaskDefinition, ContextWindowConfig, ContextOptimizationStrategy } from '../context/ContextWindowManager';
...
class IntentUnderstandingLayer {
  ...
  async parseUserIntent(
    ...
    // PSP增强的意图理解
    const enhancedPrompt = await this.buildPSPEnhancedPrompt(userInput, pspContext);
  ...
  private async buildPSPEnhancedPrompt(
    userInput: string, 
    pspContext: PSPContext,
    availableTokens: number = 4000 // 默认上下文窗口
  ): Promise<string> {
    // 使用ContextWindowManager进行智能上下文构建
    const contextManager = new ContextWindowManager({
      ...
      evictionPolicy: 'psp_guided'
    });

    // 分析当前任务
    const task: TaskDefinition = {
      id: `task_${Date.now()}`,
      type: this.analyzeTaskType(userInput),
      complexity: this.analyzeTaskComplexity(userInput),
      description: userInput,
      requirements: this.extractTaskRequirements(userInput),
      constraints: this.extractTaskConstraints(userInput, pspContext)
    };

    // 构建优化的上下文窗口
    const optimizedContext = await contextManager.buildContextWindow(
      task,
      pspContext,
      availableTokens - 500 // 为指令预留空间
    );

    // 生成增强的提示词
    return this.assembleEnhancedPrompt(optimizedContext, userInput, task);
  }

  private analyzeTaskType(userInput: string): string {
    // 简单的任务类型分析 - 实际实现会更复杂
    if (userInput.includes('搜索') || userInput.includes('查找')) return 'search';
    if (userInput.includes('创建') || userInput.includes('生成')) return 'generation';
    if (userInput.includes('分析') || userInput.includes('总结')) return 'analysis';
    return 'general';
  }

  private extractTaskRequirements(userInput: string): string[] {
    // 提取任务需求 - 实际实现会使用NLP
    const requirements: string[] = [];
    if (userInput.includes('隐私')) requirements.push('privacy_sensitive');
    if (userInput.includes('快速')) requirements.push('low_latency');
    if (userInput.includes('准确')) requirements.push('high_accuracy');
    return requirements;
  }

  private extractTaskConstraints(userInput: string, pspContext: PSPContext): Record<string, any> {
    return {
      privacyLevel: pspContext.privacy_level,
      costSensitive: pspContext.user_preferences.costSensitive || false,
      timeConstraint: this.extractTimeConstraint(userInput)
    };
  }

  private extractTimeConstraint(userInput: string): string {
    if (userInput.includes('紧急') || userInput.includes('立即')) return 'urgent';
    if (userInput.includes('快速')) return 'fast';
    return 'normal';
  }

  private assembleEnhancedPrompt(
    optimizedContext: OptimizedContext,
    userInput: string,
    task: TaskDefinition
  ): string {
    const contextSegments = optimizedContext.segments
      .sort((a, b) => b.priority - a.priority)
      .map(segment => this.formatContextSegment(segment))
      .join('\n\n');

    return `# notcontrolOS Intelligent Context Window
## Task Analysis
- Type: ${task.type}
- Complexity: ${task.complexity}
- Requirements: ${task.requirements.join(', ')}

## Optimized Context (Quality Score: ${optimizedContext.qualityScore.toFixed(2)})
${contextSegments}
...
## Instructions
Based on the optimized personal context above, determine the most appropriate tools to call.
Consider the task requirements, user preferences, and context quality in your decision.
Prioritize efficiency and relevance while respecting privacy constraints.

## Response Format
Provide your response in the standard tool calling format, with clear reasoning for tool selection.`;
  }

  private formatContextSegment(segment: ContextSegment): string {
    switch (segment.type) {
      case 'psp_master':
        return `### Personal Profile (Priority: ${segment.priority})
${this.formatPSPMasterContext(segment.content)}`;
      
      case 'psp_functional':
        return `### Active PSP: ${segment.content.name} (Priority: ${segment.priority})
${this.formatFunctionalPSPContext(segment.content)}`;
      
      case 'history':
        return `### Relevant History (Priority: ${segment.priority})
${this.formatHistoryContext(segment.content)}`;
      
      case 'tool_definitions':
        return `### Available Tools (Priority: ${segment.priority})
${this.formatToolDefinitions(segment.content)}`;
      
      default:
        return `### Context: ${segment.type} (Priority: ${segment.priority})
${JSON.stringify(segment.content, null, 2)}`;
    }
  }

  private formatPSPMasterContext(pspMaster: any): string {
    return `- Communication Style: ${pspMaster.personalProfile?.communicationStyle || 'professional'}
- Privacy Level: ${pspMaster.preferencesAndConstraints?.privacyLevel || 'standard'}
- Cost Sensitivity: ${pspMaster.preferencesAndConstraints?.costSensitive ? 'High' : 'Normal'}
- Behavioral Patterns: ${Object.keys(pspMaster.behavioralPatterns || {}).join(', ')}`;
  }

  private formatFunctionalPSPContext(functionalPSP: any): string {
    return `- Category: ${functionalPSP.category}
- Tools Integration: ${Object.keys(functionalPSP.toolsIntegration || {}).join(', ')}
- Privacy Settings: ${functionalPSP.privacySettings?.level || 'default'}`;
  }

  private formatHistoryContext(history: any): string {
    if (Array.isArray(history)) {
      return history.slice(0, 3).map((item, index) => 
        `${index + 1}. ${item.action || item.description || 'Recent action'}`
      ).join('\n');
    }
  ...
  private formatToolDefinitions(tools: any): string {
    if (Array.isArray(tools)) {
      return tools.slice(0, 5).map(tool => 
        `- ${tool.name}: ${tool.description || 'Tool available'}`
      ).join('\n');
    }
    return 'Standard tool set available';
  }
  ...
  private checkPSPPermissions(userInput: string, pspContext: PSPContext) { return { passed: true, reason: 'Permission granted' }; }
  private checkPrivacyConstraints(userInput: string, pspContext: PSPContext) { return { passed: true, reason: 'Privacy constraints satisfied' }; }
